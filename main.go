...
// Location type for GeoLocation indexes
type Location struct {
	Lat float64 `json:"lat"`
	Lon float64 `json:"lon"`
}

// Post type for Post from user
type Post struct {
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	// parce body of request into json
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Failed to parse posts into JSON", http.StatusInternalServerError)
		fmt.Printf("Failed to parse posts into JSON %v.\n", err)
		return
	}
	w.Write(js)
}
